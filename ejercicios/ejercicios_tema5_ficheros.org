...
int main()
{
  ...
  int nVocales[5] = {0};   // Para contar el numero de aparciones de cada vocal
  int i = 0, j = 0; 
  
  // Para mostrarlas en terminal en orden al finalizar el ejercicio
  ...
  while (texto[j] != '\0')
  {
	switch(texto[j]) // Compara el caracter con las vocales
    {
      case 'a':
      case 'A':
        nVocales[0]++;
  ...
  { 
    ...
    printf("%c: %d ", vocales[i], nVocales[i]);
...
Otra solución algo más difícil de entender, pero igual de válida: 
#+BEGIN_SRC C
#include <stdio.h>
...
int main()
{
  ...
  else
  {
    while (!feof(pdat))
    {
      fscanf(pdat, “%d %d %s %d”, &planta, &num, hab, &frec );
  ...
  {
    fprintf(putil, “simple %d \n cuádruple %d”, acuS, acuC);
  }
...
int main()
{
  ...
  else
  {
    ...
    {
      while(fscanf(plectura, "%d %d %s %d", &habitacion.planta, &habitacion.num, 
                                            habitacion.tipo, &habitacion.freq  ) != EOF)
      {
       ...
       printf("Suma simple: %d , suma cuadruple: %d\n", suma_sim, suma_cuad);
      fprintf(pescritura, "Suma simple: %d , suma cuadruple: %d\n", suma_sim, suma_cuad);
...
int main()
{
  ...
  else
  {
      ...
      while(fscanf(plectura, "%d;%d;%d;%d;%d", &medidas[i].num_estacion, &medidas[i].dia, &medidas[i].mes, &medidas[i].anho, &medidas[i].valor) != EOF)
      {     
        if(moratalaz == medidas[i].num_estacion)
        {
          fprintf(pescritura, "%d,%d,%d,%d\n", medidas[i].dia, medidas[i].mes, medidas[i].anho, medidas[i].valor);
        }
        i++;
      }       
  }
