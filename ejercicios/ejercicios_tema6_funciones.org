...
int main() 
{
    ...
    do 
    {
        printf("Introduce un número entero mayor que cero: ");
        scanf("%i", &n);
        if (n <= 0) 
        {
            printf("El número debe ser mayor que cero. Inténtelo de nuevo.\n");
        }
    } while (n <= 0);

    // Calcular y mostrar los n primeros términos impares de la serie
    printf("Los primeros %d términos impares de la serie son:\n", n);
    for (int i = 1; i <= n; i++) 
    {
        printf("%.2f ", TerminoN(i));
    }
    printf("\n");
