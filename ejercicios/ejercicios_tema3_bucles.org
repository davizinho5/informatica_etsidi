...
Solución 1, se basa en, para cada número, contar sus divisores (números que hacer que el resto de la división sea CERO, obviando el 1 y el mismo número). Por ejemplo, el número 6 tiene 2 divisores, el 2 y el 3. Si un número tiene algún divisor, entonces es que no es primo. 
...
Solución 2, usar una variable de tipo bandera (flag) que marca si un número es primo (valor 1), o no lo es (valor 0). PAra cada número, suponemos que el número es promo y, si encontramos un divisor, marcamos que no lo es. 
#+BEGIN_SRC C
#include <stdio.h>
...
Se aportan dos soluciones con los dos números de veces que se limita el bucle, primero 1000 veces, luego 10.000. 
...
Ahora se va a ejecutar 10.000 veces.
...
int main()
{
  ...
  // La multiplicacion se realiza fuera del bucle
  // para evitar repetir esa operacion 10000 de veces
  pi_cuadrado = 6 * pi_cuadrado;
